...
object WalkthroughSetupStatus {
    ...
    fun forStep(activity: Activity, step: WalkthroughContent.Step, settings: SettingsStore): Status? =
        when (WalkthroughActionResolver.resolve(step)) {
            WalkthroughAction.DEFAULT_PHONE -> {
            ...
            WalkthroughAction.TRANSCRIPTION_SETTINGS -> {
            ...
            WalkthroughAction.COACH_SETTINGS -> {
            ...
            WalkthroughAction.CALL_AUDIO -> when (CallAudioBridge.state(activity)) {
