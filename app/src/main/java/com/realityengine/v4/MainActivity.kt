...
class MainActivity : Activity() {
    ...
    private fun showPhone(){content.removeAllViews();content.gravity=Gravity.TOP;number=EditText(this).apply{hint="Enter number";setHintTextColor(muted);setTextColor(Color.WHITE);textSize=30f;gravity=Gravity.CENTER;background=null;inputType=android.text.InputType.TYPE_CLASS_PHONE;isSingleLine=true;setPadding(0,22.dp(),0,18.dp())};content.addView(number,LinearLayout.LayoutParams(-1,78.dp()));val grid=GridLayout(this).apply{columnCount=3;rowCount=4;alignmentMode=GridLayout.ALIGN_BOUNDS;useDefaultMargins=false};val keys=arrayOf("1","2","3","4","5","6","7","8","9","*","0","#");keys.forEach{key->val b=Button(this).apply{text=key;textSize=25f;setTextColor(Color.WHITE);background=rounded(panel,40f);stateListAnimator=null;setOnClickListener{number.append(key)}};grid.addView(b,GridLayout.LayoutParams().apply{width=0;height=72.dp();columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f);setMargins(7.dp(),7.dp(),7.dp(),7.dp())})};content.addView(grid);val actions=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER;setPadding(0,12.dp(),0,0)};actions.addView(textButton("⌫"){val t=number.text;if(t.isNotEmpty())t.delete(t.length-1,t.length)}.apply{setOnLongClickListener{number.text.clear();true}},LinearLayout.LayoutParams(68.dp(),58.dp()).apply{setMargins(4.dp(),0,8.dp(),0)});actions.addView(Button(this).apply{text="CALL";textSize=17f;setTextColor(Color.rgb(5,20,12));background=rounded(accent,28f);stateListAnimator=null;setOnClickListener{placeCall(number.text.toString().trim())}},LinearLayout.LayoutParams(0,58.dp(),1f).apply{setMargins(8.dp(),0,4.dp(),0)});content.addView(actions);error=TextView(this).apply{textSize=13f;setTextColor(muted);gravity=Gravity.CENTER;setPadding(0,12.dp(),0,0)};content.addView(error)}
    private fun showRecents(){content.removeAllViews();sectionTitle("Recents");if(checkSelfPermission(Manifest.permission.READ_CALL_LOG)!=PackageManager.PERMISSION_GRANTED){content.addView(textButton("Allow call history"){requestPermissions(arrayOf(Manifest.permission.READ_CALL_LOG),REQ_CALL_LOG)});return};val c:Cursor?=contentResolver.query(CallLog.Calls.CONTENT_URI,arrayOf(CallLog.Calls.NUMBER,CallLog.Calls.CACHED_NAME,CallLog.Calls.TYPE,CallLog.Calls.DURATION),null,null,"${CallLog.Calls.DATE} DESC");c?.use{var count=0;while(it.moveToNext()&&count<30){val n=it.getString(0)?:"Unknown";val cached=it.getString(1);val display=cached?.takeIf{s->s.isNotBlank()}?:resolveContactName(n)?:n;val type=when(it.getInt(2)){CallLog.Calls.INCOMING_TYPE->"Incoming";CallLog.Calls.OUTGOING_TYPE->"Outgoing";CallLog.Calls.MISSED_TYPE->"Missed";else->"Call"};val dur=it.getLong(3);content.addView(listButton("$display\n$type · $n · ${dur}s"){openRecent(n)}.apply{setOnLongClickListener{placeCall(n);true}});count++}}}
