...
class DeepgramStreamingClient(private val settings: SettingsStore) {
    ...
    internal fun acceptMessage(raw: String) {
        try {
            ...
            transcriptCallback?.invoke(Transcript(text, root.optBoolean("is_final"), root.optBoolean("speech_final"), speaker))
        } catch (_: Throwable) { }
    }

    internal fun endpoint(sampleRate: Int): String = Uri.Builder().scheme("wss").authority("api.deepgram.com").appendPath("v1").appendPath("listen")
        ...
        .appendQueryParameter("diarize", "true").appendQueryParameter("interim_results", "true").appendQueryParameter("smart_format", "true")
    ...
    @Synchronized private fun finish(reason: String?) {
        val wasActive = connected.getAndSet(false); socket = null
        if (wasActive || reason != null) closedCallback?.invoke(reason)
        closedCallback = null; transcriptCallback = null
    }
}
