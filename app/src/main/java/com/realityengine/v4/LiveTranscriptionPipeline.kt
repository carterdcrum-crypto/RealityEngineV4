...
class LiveTranscriptionPipeline(context: Context) {
    ...
    private fun prepareSession(onInterim:(String)->Unit,onStopped:(String?)->Unit){multichannel=false;resetSpeechState();clearTurns();acoustic.reset();callerSpeaker=null;clearPending();LiveTranscriptState.clear();interimCallback=onInterim;stoppedCallback=onStopped;conversation.bindActiveCaller()}
    private fun connectDeepgram(sampleRate:Int,channels:Int,multi:Boolean):Boolean{multichannel=multi;return deepgram.connect(sampleRate=sampleRate,channels=channels,multichannel=multi,onTranscript={result->val isCaller=if(multichannel)result.channel?.let{it==0}else{val speaker=result.speaker;if(callerSpeaker==null&&speaker!=null)callerSpeaker=speaker;speaker?.let{it==callerSpeaker}};LiveTranscriptState.publish(result.text,result.isFinal,isCaller);interimCallback?.invoke(result.text);if(result.isFinal&&result.text.isNotBlank())appendFinalTurn(result.text,isCaller!=false);if(result.speechFinal){flushTurn(isCaller!=false);if(isCaller==true)callerSpeaking=false else if(isCaller==false)userSpeaking=false}},onSpeechEvent={event->handleSpeechEvent(event)},onClosed={reason->if(running.getAndSet(false)){flushTurns();clearPending();resetSpeechState();capture.stop();stoppedCallback?.invoke(reason)}}).also{started->if(started)primeDeepgram(sampleRate,channels)}}
    ...
    private fun handleSpeechEvent(event:DeepgramStreamingClient.SpeechEvent){
        ...
        when(event){
            is DeepgramStreamingClient.SpeechEvent.Started->when(event.channel){0->{callerSpeaking=true;callerSpeechStartedAt=event.timestampSeconds};1->{userSpeaking=true;userSpeechStartedAt=event.timestampSeconds}}
            is DeepgramStreamingClient.SpeechEvent.Ended->when(event.channel){0->if(callerSpeechStartedAt<0.0||event.lastWordEndSeconds>=callerSpeechStartedAt){callerSpeaking=false;flushTurn(true)};1->if(userSpeechStartedAt<0.0||event.lastWordEndSeconds>=userSpeechStartedAt){userSpeaking=false;flushTurn(false)}}
        }
    }
    private fun resetSpeechState(){callerSpeaking=false;userSpeaking=false;callerSpeechStartedAt=-1.0;userSpeechStartedAt=-1.0}

    private fun primeDeepgram(sampleRate:Int,channels:Int){Thread({val deadline=System.currentTimeMillis()+FIRST_AUDIO_DEADLINE_MS;while(running.get()&&!deepgram.isConnected()&&System.currentTimeMillis()<deadline){try{Thread.sleep(25)}catch(_:InterruptedException){return@Thread}};if(!running.get()||!deepgram.isConnected())return@Thread;deepgram.sendPcm(ByteArray((sampleRate*channels*2)/10))},"reality-deepgram-prime").apply{isDaemon=true;start()}}
    ...
    fun stop(){if(!running.getAndSet(false))return;flushTurns();clearPending();capture.stop();deepgram.close();conversation.clear();acoustic.reset();acousticScore=0;callerSpeaker=null;resetSpeechState();clearTurns();multichannel=false;interimCallback=null;stoppedCallback=null;LiveTranscriptState.clear()}
