...
class LiveTranscriptionPipeline(context: Context) {
    ...
    @Volatile private var interimCallback:((String)->Unit)?=null;@Volatile private var stoppedCallback:((String?)->Unit)?=null;@Volatile private var acousticScore=0;@Volatile private var callerSpeaker:Int?=null
    private val pendingLock=Any();private val pendingPcm=ByteArrayOutputStream(MAX_PENDING_PCM)
    ...
    fun start(onInterim:(String)->Unit={},onStopped:(String?)->Unit={}):StartResult{
        ...
        return when(val result=capture.start(onPcm={bytes,length->if(running.get()){acousticScore=acoustic.analyze(bytes,length).score;sendOrBuffer(bytes,length)}},onStopped={reason->if(running.getAndSet(false)){clearPending();deepgram.close();stoppedCallback?.invoke(reason)}})){
    ...
    fun startTwilio(onInterim:(String)->Unit={},onStopped:(String?)->Unit={}):StartResult=if(begin(8_000,onInterim,onStopped))StartResult.Started else StartResult.Unavailable(startFailureReason())
    ...
    fun stop(){if(!running.getAndSet(false))return;clearPending();capture.stop();deepgram.close();conversation.clear();acoustic.reset();acousticScore=0;callerSpeaker=null;interimCallback=null;stoppedCallback=null;LiveTranscriptState.clear()}
    fun isRunning():Boolean=running.get()
    companion object{private const val MAX_PENDING_PCM=64_000}
}
