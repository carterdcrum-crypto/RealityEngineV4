...
class LiveTranscriptionPipeline(context: Context) {
    ...
    fun start(onInterim:(String)->Unit={},onStopped:(String?)->Unit={}):StartResult{
        val dual=ShizukuAudioStatus.state()==ShizukuAudioStatus.State.READY
        if(!begin(VoiceCallPcmCapture.SAMPLE_RATE,onInterim,onStopped,channels=if(dual)2 else 1,multichannel=dual))return StartResult.Unavailable(startFailureReason())
        return when(val result=capture.start(onFrame={bytes,length,direction->if(running.get()){when(direction){VoiceCallPcmCapture.Direction.DOWNLINK->acceptDirectional(bytes,length,true);VoiceCallPcmCapture.Direction.UPLINK->acceptDirectional(bytes,length,false);VoiceCallPcmCapture.Direction.MIXED->{acousticScore=acoustic.analyze(bytes,length).score;sendOrBuffer(bytes,length)}}}},onStopped={reason->if(running.getAndSet(false)){clearPending();deepgram.close();stoppedCallback?.invoke(reason)}})){
