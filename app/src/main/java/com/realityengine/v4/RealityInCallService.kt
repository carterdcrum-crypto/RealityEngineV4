...
class RealityInCallService : InCallService() {
    ...
    private fun syncTranscription(){
        ...
        if(call.state!=Call.STATE_ACTIVE){if(transcription.isRunning())transcription.stop();AudioRouteState.clear();return}
        val decision=audioRouter.decide(twilioCallActive=TwilioFallbackState.isActive());AudioRouteState.publish(decision);AudioRouteState.diagnose(applicationContext);val diagnostic=AudioRouteState.snapshot().detail
        when(decision.route){
            AudioCaptureRouter.Route.SHIZUKU_VOICE_CALL,
            AudioCaptureRouter.Route.NATIVE_VOICE_COMMUNICATION -> if(!transcription.isRunning()){LiveTranscriptState.publish("CALL AUDIO // $diagnostic",false);transcription.start()}
            AudioCaptureRouter.Route.TWILIO_MEDIA_STREAM -> if(!transcription.isRunning()){LiveTranscriptState.publish("CALL AUDIO // $diagnostic",false);transcription.startTwilio()}
    ...
    private val callback=object:Call.Callback(){override fun onStateChanged(call:Call,state:Int){if(state==Call.STATE_DISCONNECTED){val endedNumber=CallSessionRegistry.numberFor(call).orEmpty();CallSessionRegistry.removeIfDisconnected(call);finalizeOnce(call,endedNumber);if(CallSessionRegistry.primary()==null){transcription.stop();LiveTranscriptState.clear();LiveSignalState.clear();AudioRouteState.clear()}else syncTranscription()}else{CallSessionRegistry.add(call);syncTranscription();launchCallUi()}}}
}
