 ...
 * Protected VOICE_CALL audio is never opened from the ordinary app process. */
class VoiceCallPcmCapture(context: Context) {
    ...
    fun start(onPcm: (ByteArray, Int) -> Unit, onStopped: (String?) -> Unit = {}): StartResult {
        ...
        if (!ShizukuAudioClient.connect()) {
            running.set(false)
            return StartResult.Unavailable("Privileged audio service unavailable")
        }
        val startCode = ShizukuAudioClient.start()
        if (startCode != PrivilegedAudioService.START_OK) {
            running.set(false)
            ShizukuAudioClient.disconnect()
            return StartResult.Unavailable("VOICE_CALL unavailable in privileged service")
        }

        activeSource = "SHIZUKU_VOICE_CALL"
        worker = Thread({
            var failure: String? = null
            try {
                while (running.get()) {
                    val pcm = ShizukuAudioClient.read(READ_CHUNK_BYTES)
                ...
                if (running.get()) failure = "Privileged call audio stream stopped unexpectedly"
            } finally {
                running.set(false)
                activeSource = null
                ShizukuAudioClient.disconnect()
        ...
        }, "reality-shizuku-call-pcm").apply { isDaemon = true; start() }
        return StartResult.Started(Format(), "SHIZUKU_VOICE_CALL")
    }
    ...
    companion object {
        ...
        private const val READ_IDLE_MS = 10L
    }
}
