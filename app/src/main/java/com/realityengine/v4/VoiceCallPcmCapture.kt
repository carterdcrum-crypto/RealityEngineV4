...
class VoiceCallPcmCapture(context:Context){
 ...
 fun start(onPcm:(ByteArray,Int)->Unit,onStopped:(String?)->Unit={}):StartResult=start(onFrame={bytes,length,_->onPcm(bytes,length)},onStopped=onStopped)
 fun start(onFrame:(ByteArray,Int,Direction)->Unit,onStopped:(String?)->Unit={}):StartResult{
  ...
  val source=ShizukuAudioClient.activeSourceLabel();val dual=ShizukuAudioClient.captureMode()==PrivilegedAudioService.CAPTURE_MODE_DUAL;activeSource="SHIZUKU_$source"
