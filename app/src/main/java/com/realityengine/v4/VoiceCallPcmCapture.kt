...
class VoiceCallPcmCapture(context:Context){
 ...
 fun start(onPcm:(ByteArray,Int)->Unit,onStopped:(String?)->Unit={}):StartResult{
  ...
  val startCode=ShizukuAudioClient.start();if(startCode!=PrivilegedAudioService.START_OK){running.set(false);ShizukuAudioClient.disconnect();return StartResult.Unavailable("VOICE_CALL unavailable in privileged service (code $startCode)")}
