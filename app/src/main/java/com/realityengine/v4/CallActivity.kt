...
class CallActivity : Activity() {
    ...
    private val handler=Handler(Looper.getMainLooper());private var activeStartedAt:Long?=null
    ...
    private fun buildUi(){
        ...
        muteButton=Button(this).apply{setOnClickListener{toggleMute()}};speakerButton=Button(this).apply{setOnClickListener{toggleSpeaker()}};holdButton=Button(this).apply{setOnClickListener{toggleHold()}}
        ...
        keypadContainer=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;visibility=android.view.View.GONE};val digits=arrayOf("1","2","3","4","5","6","7","8","9","*","0","#");val grid=GridLayout(this).apply{columnCount=3;useDefaultMargins=true}
    ...
    private fun refresh(){
        call=CallSessionRegistry.primary();val current=call;if(current==null){finish();return}
        val number=current.details?.handle?.schemeSpecificPart?:"UNKNOWN CALLER";caller.text=resolveCallerLabel(number)
        ...
        holdButton.text=if(current.state==Call.STATE_HOLDING)"RESUME" else "HOLD";holdButton.isEnabled=current.state==Call.STATE_ACTIVE||current.state==Call.STATE_HOLDING
        if(current.state==Call.STATE_ACTIVE&&activeStartedAt==null)activeStartedAt=SystemClock.elapsedRealtime();if(current.state==Call.STATE_DISCONNECTED)activeStartedAt=null;updateTimer()
