...
class CallActivity : Activity() {
    ...
    private fun buildUi(){
        ...
        val incoming=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER};answerButton=Button(this).apply{text="ANSWER";setOnClickListener{call?.takeIf{it.state==Call.STATE_RINGING}?.answer(0)}};rejectButton=Button(this).apply{text="DECLINE";setOnClickListener{call?.takeIf{it.state==Call.STATE_RINGING}?.reject(false,null)}}
        ...
        val controls=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER};muteButton=Button(this).apply{setOnClickListener{toggleMute()}};speakerButton=Button(this).apply{setOnClickListener{toggleSpeaker()}};bluetoothButton=Button(this).apply{setOnClickListener{toggleBluetooth()}};holdButton=Button(this).apply{setOnClickListener{toggleHold()}}
        ...
        root.addView(Button(this).apply{text="KEYPAD";setOnClickListener{keypadContainer.visibility=if(keypadContainer.visibility==View.VISIBLE)View.GONE else View.VISIBLE}});keypadContainer=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;visibility=View.GONE};val digits=arrayOf("1","2","3","4","5","6","7","8","9","*","0","#");val grid=GridLayout(this).apply{columnCount=3;useDefaultMargins=true}
        digits.forEach{digit->grid.addView(Button(this).apply{text=digit;textSize=20f;minWidth=0;minHeight=0;setOnClickListener{sendDtmf(digit[0])}},GridLayout.LayoutParams().apply{width=0;height=56.dp();columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f)})};keypadContainer.addView(grid);root.addView(keypadContainer);root.addView(Button(this).apply{text="END CALL";setOnClickListener{call?.disconnect()}});setContentView(root)
    }
    ...
    private fun refreshAudioButtons(){val audio=RealityInCallService.instance?.callAudioState;speakerButton.text=if(audio?.route==CallAudioState.ROUTE_SPEAKER)"EARPIECE" else "SPEAKER";bluetoothButton.text=if(audio?.route==CallAudioState.ROUTE_BLUETOOTH)"BT OFF" else "BLUETOOTH";bluetoothButton.isEnabled=(audio?.supportedRouteMask?:0) and CallAudioState.ROUTE_BLUETOOTH!=0}
