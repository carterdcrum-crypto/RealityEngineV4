...
class CallActivity : Activity() {
    ...
    private fun buildUi(){
        ...
        digits.forEach{digit->grid.addView(Button(this).apply{text=digit;textSize=20f;minWidth=0;minHeight=0;setOnTouchListener{_,event->when(event.actionMasked){MotionEvent.ACTION_DOWN->{call?.playDtmfTone(digit[0]);true};MotionEvent.ACTION_UP,MotionEvent.ACTION_CANCEL->{call?.stopDtmfTone();performClick();true};else->false}}},GridLayout.LayoutParams().apply{width=0;height=56.dp();columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f)})};keypadContainer.addView(grid);root.addView(keypadContainer);root.addView(Button(this).apply{text="END CALL";setOnClickListener{call?.disconnect()}});setContentView(root)
    }
