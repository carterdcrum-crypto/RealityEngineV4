...
object ShizukuAudioClient {
    ...
    fun activeSourceLabel():String=when(activeSource()){android.media.MediaRecorder.AudioSource.VOICE_CALL->"VOICE_CALL";android.media.MediaRecorder.AudioSource.VOICE_DOWNLINK->"VOICE_DOWNLINK";android.media.MediaRecorder.AudioSource.VOICE_UPLINK->"VOICE_UPLINK";else->"UNKNOWN"}
    fun readResult(maxBytes:Int):ReadResult{val binder=remote?.takeIf{it.isBinderAlive}?:return ReadResult.Failed(PrivilegedAudioService.READ_DEAD_OBJECT);val data=Parcel.obtain();val reply=Parcel.obtain();return try{data.writeInterfaceToken(PrivilegedAudioService.DESCRIPTOR);data.writeInt(maxBytes.coerceIn(320,PrivilegedAudioService.MAX_BINDER_CHUNK));if(!binder.transact(PrivilegedAudioService.TRANSACTION_READ,data,reply,0))return ReadResult.Failed(PrivilegedAudioService.READ_FAILED);reply.readException();val count=reply.readInt();when{count>0->{val bytes=reply.createByteArray() ?: ByteArray(0);ReadResult.Pcm(if(bytes.size==count)bytes else bytes.copyOf(count.coerceAtMost(bytes.size)))};count==0->ReadResult.Empty;else->ReadResult.Failed(count)}}catch(_:Throwable){ReadResult.Failed(PrivilegedAudioService.READ_FAILED)}finally{reply.recycle();data.recycle()}}
    ...
    private fun transactInt(code:Int):Int{val binder=remote?.takeIf{it.isBinderAlive}?:return PrivilegedAudioService.START_SOURCE_BLOCKED;val data=Parcel.obtain();val reply=Parcel.obtain();return try{data.writeInterfaceToken(PrivilegedAudioService.DESCRIPTOR);if(!binder.transact(code,data,reply,0))PrivilegedAudioService.START_SOURCE_BLOCKED else{reply.readException();when(code){PrivilegedAudioService.TRANSACTION_START,PrivilegedAudioService.TRANSACTION_STATUS->reply.readInt();else->PrivilegedAudioService.START_OK}}}catch(_:Throwable){PrivilegedAudioService.START_SOURCE_BLOCKED}finally{reply.recycle();data.recycle()}}
}
