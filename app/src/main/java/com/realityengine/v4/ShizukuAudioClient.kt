...
object ShizukuAudioClient {
    ...
    fun startScrcpy(serverPath: String): ParcelFileDescriptor? {
        ...
        return try {
            ...
            if (!binder.transact(PrivilegedAudioService.TRANSACTION_START_SCRCPY, data, reply, 0)) return null
            reply.readException()
            if (reply.readInt() == 0) null else ParcelFileDescriptor.CREATOR.createFromParcel(reply)
        } catch (_: Throwable) {
            null
        } finally {
    ...
    fun probeSummary(): String {
        ...
        val attempted = buildList {
            if (mask and PrivilegedAudioService.PROBE_VOICE_CALL != 0) add("VOICE_CALL")
            if (mask and PrivilegedAudioService.PROBE_VOICE_DOWNLINK != 0) add("VOICE_DOWNLINK")
            if (mask and PrivilegedAudioService.PROBE_VOICE_UPLINK != 0) add("VOICE_UPLINK")
            if (mask and PrivilegedAudioService.PROBE_VOICE_COMMUNICATION != 0) add("VOICE_COMMUNICATION")
        }
        return if (mask and PrivilegedAudioService.PROBE_DUAL_SUCCESS != 0) "Protected dual RX/TX active"
        else if (mask and PrivilegedAudioService.PROBE_SUCCESS != 0) "Protected source active: ${activeSourceLabel()}"
        else if (attempted.isEmpty()) "No protected source probe completed"
        else "Protected sources blocked: ${attempted.joinToString(" / ")}"
    }
    ...
    private fun readTransaction(code: Int, maxBytes: Int): ReadResult {
        val binder = remote?.takeIf { it.isBinderAlive } ?: return ReadResult.Failed(PrivilegedAudioService.READ_DEAD_OBJECT)
        val data = Parcel.obtain(); val reply = Parcel.obtain()
        return try {
            data.writeInterfaceToken(PrivilegedAudioService.DESCRIPTOR)
            data.writeInt(maxBytes.coerceIn(320, PrivilegedAudioService.MAX_BINDER_CHUNK))
            if (!binder.transact(code, data, reply, 0)) return ReadResult.Failed(PrivilegedAudioService.READ_FAILED)
            reply.readException()
            val count = reply.readInt()
            when {
                count > 0 -> {
                    val bytes = reply.createByteArray() ?: ByteArray(0)
                    ReadResult.Pcm(if (bytes.size == count) bytes else bytes.copyOf(count.coerceAtMost(bytes.size)))
                }
                count == 0 -> ReadResult.Empty
                else -> ReadResult.Failed(count)
            }
        } catch (_: Throwable) { ReadResult.Failed(PrivilegedAudioService.READ_FAILED) }
        finally { reply.recycle(); data.recycle() }
    }

    private fun unbindLocked(connection: ServiceConnection) {
    ...
    private fun transactInt(code: Int): Int {
        ...
        return try {
            ...
            if (!binder.transact(code, data, reply, 0)) PrivilegedAudioService.START_SOURCE_BLOCKED
                ...
                reply.readException()
                when (code) {
                    PrivilegedAudioService.TRANSACTION_START,
                    PrivilegedAudioService.TRANSACTION_STATUS,
                    PrivilegedAudioService.TRANSACTION_PROBE_STATUS,
                    PrivilegedAudioService.TRANSACTION_CAPTURE_MODE -> reply.readInt()
                    else -> PrivilegedAudioService.START_OK
                }
            }
